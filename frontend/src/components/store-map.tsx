    ...
    <div className="cdx-map relative h-[500px] w-full overflow-hidden rounded-tl-[2rem] rounded-br-[2rem] md:h-[440px]">
      {isLoaded && coords ? (
        <GoogleMap
          ...
          <iframe
            src={`https://maps.google.com/maps?q=${FALLBACK_CENTER.lat},${FALLBACK_CENTER.lng}&z=16&output=embed`}
        ...
        {/* Floating panel */}
        <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex justify-center p-3 md:pointer-events-auto md:inset-auto md:left-10 md:top-1/2 md:w-[19rem] md:-translate-y-1/2 md:p-0">
          <div className="pointer-events-auto w-full max-w-[19rem] rounded-tl-2xl rounded-br-2xl border-2 border-brand-200 bg-white/95 p-5 shadow-2xl shadow-brand-500/10 backdrop-blur md:max-w-none md:p-6">
            <p className="text-sm font-medium text-slate-600">Encuentranos en</p>
            <h3 className="mt-1 text-2xl font-bold tracking-tight gradient-text">{STORE.name}</h3>

            <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-brand-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-brand-700">
